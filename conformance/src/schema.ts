...
export class SchemaRegistry {
  ...
  genericErrorResponse(
    ...
    const validator = this.memoizedValidator(state, "generic-error-response", {
  ...
  private validate(
    ...
    const direction =
      sender === "client" ? "client-to-agent" : "agent-to-client";
    ...
    const validator = this.memoizedValidator(
      state,
      key,
      this.buildEnvelopeSchema(version, sender, kind, message, responseMethod),
    );
    if (!validator(message)) {
      const method =
        kind === "response"
          ? (responseMethod ?? "unknown-response")
          : (message as JsonRpcRequest | JsonRpcNotification).method;
      throw this.validationError(
        version,
        method,
        direction,
  ...
  private buildEnvelopeSchema(
    ...
    if (kind === "response") {
      if ("error" in message) {
        return {
          ...
          properties: {
            ...
            id: {
              anyOf: [
                { $ref: this.defRef(version, "RequestId") },
                { type: "null" },
              ],
            },
            error: {
      ...
      if (!method) {
        throw new Error(
          "Missing response method context for schema validation",
        );
      }
      const defName =
        state.methods.responseBySender[sender].get(method) ?? "ExtResponse";
      return {
        type: "object",
        properties: {
          jsonrpc: { const: "2.0" },
    ...
    const map =
      kind === "request"
        ? state.methods.requestBySender[sender]
        : state.methods.notificationBySender[sender];
  ...
  private memoizedValidator(
    state: VersionSchemaState,
    key: string,
    schema: Record<string, unknown>,
  ): ValidateFunction {
  ...
  private indexMethods(schema: Record<string, unknown>): MethodMaps {
    ...
    for (const [defName, def] of Object.entries(defs)) {
      const method =
        typeof def["x-method"] === "string"
          ? (def["x-method"] as string)
          : undefined;
      ...
      if (
        defName.endsWith("Request") &&
        (side === "client" || side === "agent")
      ) {
        methods.requestBySender[side === "agent" ? "client" : "agent"].set(
          method,
          defName,
        );
      } else if (
        defName.endsWith("Response") &&
        (side === "client" || side === "agent")
      ) {
        ...
        if (method.startsWith("$/")) {
          ...
          methods.notificationBySender[
            side === "agent" ? "client" : "agent"
          ].set(method, defName);
        }
      }
    }
  ...
  private validationError(
    ...
    const details: ValidationFailure = {
      ...
      path,
      expected,
      actual: stringifyValue(
        path === "/" ? message : this.lookup(message, path),
      ),
      rawMessage: raw,
    };
    const error = new Error(
      `[${direction}] ${method} failed schema validation at ${path}: expected ${details.expected}, got ${details.actual}`,
    ) as Error & {
