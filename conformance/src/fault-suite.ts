...
async function main(): Promise<void> {
  ...
  for (const profile of referenceFaultProfiles) {
    ...
    const summary = await runSuite(
      builtinProfile("reference-stdio", profile.faults),
      {
        expectedFail: true,
    ...
    if (summary.reportPath) {
      ...
      const report = JSON.parse(reportText) as {
        failed: number;
        aborted: boolean;
        cases: Array<{ failure?: { message?: string } }>;
      };
...
async function runInvalidFaultProbe(): Promise<{
  ok: boolean;
  ...
  const ok =
    exitCode !== 0 &&
    /Unknown reference fault name\(s\): this-name-does-not-exist/i.test(
      combined,
    ) &&
    /Valid faults:/i.test(combined);
  return {
    ok,
    exitCode,
